 ...
 * damage value, exactly like vanilla's material repair does for ingots.
 */
public class RepairKitRecipe extends SpecialCraftingRecipe {
    public static final RecipeSerializer<RepairKitRecipe> SERIALIZER =
            RecipeSerializer.register("equip_leveling:repair_kit",
                    new SpecialRecipeSerializer<>(RepairKitRecipe::new));

    public RepairKitRecipe(CraftingRecipeCategory category) {
        super(category);
    }
    ...
    public ItemStack craft(CraftingRecipeInput input, RegistryWrapper.WrapperLookup lookup) {
        ...
        // Restore a configurable percentage of max durability (default 100%).
        int restore = (int) Math.round(result.getMaxDamage()
                * (EquipLevelingConfig.getRepairKitRestorePercent() / 100.0));
